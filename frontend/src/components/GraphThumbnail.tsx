...
function build(id: string): Geo | null {
  ...
  // edges: side-to-side curves (data flows left->right); skip visual-only fusion beams
  const paths: string[] = [];
...
export function GraphThumbnail({ id, rev }: { id: string; rev: number }) {
  ...
  return (
    <svg className="mcard-thumb" viewBox={geo.viewBox} preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      ...
      {geo.rects.map((r) => (
        <rect key={r.id} className="mt-node" x={r.x} y={r.y} width={r.w} height={r.h} rx={5} style={{ fill: r.fill }} />
      ))}
    </svg>
  );
}
