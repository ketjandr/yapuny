...
export const NODE_CATALOG: Record<string, NodeDef> = {
  token_embedding: {
    ...
    description: "Looks up a learned `C`-dim vector for each token id from the `vocab_size×C` embedding table.",
    variant: "req",
    inputs: [p("idx", ["T"])],
  ...
  mlp_up: {
    ...
    label: "MLP Up",
    subtitle: "out = x·W_up",
    description: "Linear layer widening features from `C` to `4C`, the expansion half of the feed-forward block.",
    variant: "req",
    inputs: [p("x", ["T", "C"])],
    outputs: [p("out", ["T", "4C"])],
    fusable: true,
  ...
  mlp_down: {
    ...
    description: "Linear layer projecting the `4C` hidden back down to `C`, completing the feed-forward block.",
    variant: "req",
    inputs: [p("x", ["T", "4C"])],
    outputs: [p("out", ["T", "C"])],
    fusable: true,
